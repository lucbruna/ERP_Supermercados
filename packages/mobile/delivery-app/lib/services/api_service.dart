...
class ApiService {
  ...
  Future<void> setTokens(String token, String refreshToken) async {
    ...
    await _secureStorage.write(key: 'delivery_jwt_token', value: token);
    await _secureStorage.write(key: 'delivery_refresh_token', value: refreshToken);
  }

  Future<void> loadTokens() async {
    _token = await _secureStorage.read(key: 'delivery_jwt_token');
    _refreshToken = await _secureStorage.read(key: 'delivery_refresh_token');
  ...
  Future<void> clearTokens() async {
    _token = null;
    _refreshToken = null;
    await _secureStorage.delete(key: 'delivery_jwt_token');
    await _secureStorage.delete(key: 'delivery_refresh_token');
  }
  ...
  Future<dynamic> uploadBytes(String endpoint, List<int> bytes, String fileName, {String fieldName = 'file'}) async {
    final uri = Uri.parse('${ApiConfig.baseUrl}$endpoint');
    final request = http.MultipartRequest('POST', uri);
    request.headers.addAll(_buildHeaders());
