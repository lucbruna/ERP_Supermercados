...
export default function LineChart({ data, lines, xAxisKey, title, height = 300 }: LineChartProps) {
  ...
  return (
    <Box>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <RechartsLineChart data={data as Record<string, string | number>[]} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
          ...
          <Legend />
          {lines.map((line, idx) => (
            <Line key={line.dataKey} type="monotone" dataKey={line.dataKey} stroke={line.color || colors[idx % colors.length]} name={line.name} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
          ))}
        </RechartsLineChart>
      </ResponsiveContainer>
    </Box>
